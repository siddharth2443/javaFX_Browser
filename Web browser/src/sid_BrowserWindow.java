...
/**
 * This package-private class defines one sid_browser window for use with 
 ...
 */
class sid_BrowserWindow extends Stage {

    private sid_WebBrowser sid_owner;    // the WebBrowser application
    private WebEngine sid_webEngine; // loads and manages pages
    private Menu sid_windowMenu;     // holds window-related commands

    /**
     * The constructor sets up the window, and starts loading the
     * initial web page location, if given.  It does not show the window.
     * @param sid_browser     the WebBrowser application that is createing this window
     * @param sid_initialURL  if non-null, the window will attempt to load this URL
     */
    sid_BrowserWindow(sid_WebBrowser sid_browser, String sid_initialURL) {
        
        sid_owner = sid_browser;
        ...
        sid_webEngine = webview.getEngine();
        ...
        sid_webEngine.locationProperty().addListener( (o,oldVal,newVal) -> {
        ...
        sid_webEngine.titleProperty().addListener( (o,oldVal,newVal) -> {
            if (newVal == null)
                setTitle("Untitled " + sid_owner.sid_getNextUntitledCount());
            else
                setTitle(newVal); 
        });
        ...
        sid_webEngine.getLoadWorker().stateProperty().addListener( (o,oldVal,newVal) -> {
        ...
        cancelButton.setOnAction( e -> { // Cancel the ongoing page load
            if ( sid_webEngine.getLoadWorker().getState() == Worker.State.RUNNING )
                sid_webEngine.getLoadWorker().cancel();
        ...
        sid_webEngine.setOnAlert( evt -> SimpleDialogs.message(evt.getData(), "Alert from web page") );
        sid_webEngine.setPromptHandler( promptData -> 
                    SimpleDialogs.prompt(promptData.getMessage(), 
                                              "Query from web page", promptData.getDefaultValue()));
        sid_webEngine.setConfirmHandler( str -> 
                    SimpleDialogs.confirm(str, "Confirmation Needed").equals("yes") );
        
        /* If the sid_initialURL is not null, load that page! */
        
        if (sid_initialURL != null) {
               // load the initial web page, if any
            doLoad(sid_initialURL);
        }
    ...
    private MenuBar makeMenuBar() {
        ...
        newWin.setOnAction( e -> sid_owner.sid_newBrowserWindow(null) );
        ...
        open.setOnAction( e -> {
            ...
            if (url != null && url.trim().length() > 0)
                sid_owner.sid_newBrowserWindow(url);
        });
        sid_windowMenu = new Menu("Window");
        sid_windowMenu.getItems().addAll(newWin,close,open,new SeparatorMenuItem());
        sid_windowMenu.setOnShowing( e -> populateWindowMenu() );
        MenuBar menubar = new MenuBar(sid_windowMenu);
        return menubar;
    }
    ...
    private void populateWindowMenu() {
        ArrayList<sid_BrowserWindow> windows = sid_owner.getOpenWindowList();
        while (sid_windowMenu.getItems().size() > 4) {
            ...
            sid_windowMenu.getItems().remove(sid_windowMenu.getItems().size() - 1);
        }
        if (windows.size() > 1) {
            ...
            sid_windowMenu.getItems().add(item);
            sid_windowMenu.getItems().add( new SeparatorMenuItem() );
        }
        for (sid_BrowserWindow window : windows) {
            ...
            final sid_BrowserWindow win = window; // (for use in a lambda expression)
